...
class advanced_search_plugin extends Plugin
{
	...
	function GetDefaultSettings()
	{
		return array(
				...
				'posts_per_page_live' => array(
					'label' => $this->T_('Live search results'),
					'defaultvalue' => 3,
					'type' => 'integer',
					'size' => 4,
					'note' => $this->T_('Number of results in pop-up window.'),
				),
				'hi_class' => array(
	...
	*/
	function get_widget_param_definitions( $params )
	{
		return array(
				'popup_results' => array(
					'label' => $this->T_('Pop-up results'),
					'defaultvalue' => true,
					'type' => 'checkbox',
					'note' => $this->T_('Display live search results in pop-up window.'),
				),
				'search_title' => array(
	...
	function SessionLoaded()
	{
		...
		// This is a workaround for b2evo 3.x where we can't pass GET requests
	...
	{
		global $Blog, $DB, $posts, $types, $skin;
		...
		if( $author = param( 'author', 'integer', '' ) )
		{
			...
			if( !empty($wi_params) )
			{
				...
				$author_exclude = array_filter( $this->array_trim( explode( ',', $wi_params['author_exclude'] ) ) );
		...
		if ( $s = param( 's', 'string' ) )
		{
			...
			if( param( 'search_query', 'string' ) )
			{
				$skin = 'search_query_skin';
			}
		}
		...
		if( $in_blogs = param( 'in_blogs', 'string' ) )
		{
			...
			{	// Sanitize the list
				$this->in_blogs = implode( ',', array_filter( $this->array_trim( explode( ',', $in_blogs ) ) ) );
			}
	...
	{
		global $MainList, $disp;

		skin_init( $disp );

		$total = $MainList->total_rows;
		$limit = $this->Settings->get('posts_per_page_live');;

		echo '<ol id="search_popup_list">';

		if( $total < 1 )
		{
			echo '<div class="search_results_no">No Results</div>';
		}

		$i = 0;
		while( $Item = & $MainList->get_item() && $i < $limit )
		{
			echo '<li class="search_popup_item"><a href="'.$Item->get_permanent_url().'">'.$Item->get_title().'</a></li>';
		...
		if( $total > $limit )
		{
			echo '<div class="search_popup_all">
					<a href="'.regenerate_url( 'search_query,posts' ).'">'.
					sprintf( $this->T_('Show all %s results'), $total ).'</a></div>';
		}
	}
	...
	function SkinBeginHtmlHead()
	{
		...
		require_css( $plugins_url.'advanced_search_plugin/adv_search.css', true );
		require_js( $plugins_url.'advanced_search_plugin/prototype.js', true );
		require_js( $plugins_url.'advanced_search_plugin/adv_search.js', true );

		$js_headline = 'ls.url = "'.url_add_param( $Blog->gen_blogurl(), 'search_query=true' ).'";';
		$css_headline = '
		.search_popup_top { background: #fff url( "'.$plugins_url.'advanced_search_plugin/pop-up-top.png") top left no-repeat }
		.search_popup_bottom { background: #fff url( "'.$plugins_url.'advanced_search_plugin/pop-up-bottom.png") bottom left no-repeat }';
	...
	function SkinTag( $params )
	{
		...
		// Pop-up results:
		if(!isset($params['popup_results'])) $params['popup_results'] = true;


		if( ! empty($params['category_exclude']) )
		{	// Exclude categories
			$category_exclude = array_filter( $this->array_trim( explode( ',', $params['category_exclude'] ) ) );
		}
		else
		{
			$category_exclude = array();
		...
		{	// Exclude authors
			$author_exclude = array_filter( $this->array_trim( explode( ',', $params['author_exclude'] ) ) );
		}
		...
		echo '<input id="s" type="text" name="s" size="25" value="'.htmlspecialchars( get_param('s') ).'" class="SearchField" />';

		if( $params['popup_results'] )
		{	//<div class="search_popup_types"><a href="#"><a href="#">All posts</a> / <a href="#">All posts</a> / <a href="#">All posts</a></div>
			echo '<div id="search_popup_box" style="display: none">
					<div class="search_popup_top">
					  <div id="search_popup_results"></div>
					</div>
		...
		if( $params['in_blogs'] )
		{	// Search in Blogs
			$Form->hidden( 'in_blogs', implode( ',', array_filter( $this->array_trim( explode( ',', $params['in_blogs'] ) ) ) ) );
		}
